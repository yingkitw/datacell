use datacell::{AvroHandler, ParquetHandler};
use std::fs;
use std::path::Path;
...
fn test_read_parquet_employees_example() {
    let handler = ParquetHandler::new();
    let data = handler
        .read_with_headers("examples/employees.parquet")
        .unwrap();
...
fn test_read_parquet_numbers_example() {
    let handler = ParquetHandler::new();
    let data = handler
        .read_with_headers("examples/numbers.parquet")
        .unwrap();
...
fn test_read_avro_employees_example() {
    let handler = AvroHandler::new();
    let data = handler
        .read_with_headers("examples/employees.avro")
        .unwrap();
