...
use datacell::{CsvHandler, CellRange, StreamingCsvReader, StreamingCsvWriter};
use std::fs;
...
fn test_csv_handler_read_as_json() {
    let handler = CsvHandler::new();
    let json = handler.read_as_json("examples/numbers.csv").unwrap();
...
fn test_csv_handler_read_range() {
    let handler = CsvHandler::new();
    let range = CellRange::parse("A1:B2").unwrap();
    let data = handler.read_range("examples/numbers.csv", &range).unwrap();
...
fn test_streaming_csv_writer() {
    ...
    
    {
        ...
        writer.write_row(&["1".to_string(), "2".to_string()]).unwrap();
        writer.write_row(&["3".to_string(), "4".to_string()]).unwrap();
