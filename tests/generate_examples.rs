...
fn generate_parquet_examples() {
    ...
    handler
        .write("examples/sales.parquet", &data, Some(&header))
        .unwrap();
    ...
    handler
        .write("examples/employees.parquet", &data, Some(&header))
        .unwrap();
    ...
    handler
        .write("examples/numbers.parquet", &data, Some(&header))
        .unwrap();
...
fn generate_avro_examples() {
    ...
    let data: Vec<Vec<String>> = sales[1..].to_vec();
    handler
        .write("examples/sales.avro", &data, Some(&header))
        .unwrap();
    ...
    let data: Vec<Vec<String>> = employees[1..].to_vec();
    handler
        .write("examples/employees.avro", &data, Some(&header))
        .unwrap();
    ...
    handler
        .write("examples/lookup.avro", &data, Some(&header))
        .unwrap();
...
fn generate_excel_examples() {
    ...
    handler
        .write_styled("examples/sales.xlsx", &sales, &options)
        .unwrap();
    ...
    let employees = read_csv("employees");
    handler
        .write_styled("examples/employees.xlsx", &employees, &options)
        .unwrap();
