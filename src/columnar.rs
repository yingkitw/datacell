...
use arrow::array::{ArrayRef, StringArray, Float64Array, Int64Array, BooleanArray};
...
impl ParquetHandler {
    ...
    pub fn write(&self, path: &str, data: &[Vec<String>], column_names: Option<&[String]>) -> Result<()> {
        if data.is_empty() {
...
impl AvroHandler {
    ...
    pub fn write(&self, path: &str, data: &[Vec<String>], field_names: Option<&[String]>) -> Result<()> {
        if data.is_empty() {
...
mod tests {
    ...
    fn test_parquet_write_read() {
        ...
        handler.write(path, &data, Some(&["name".to_string(), "value".to_string()])).unwrap();
