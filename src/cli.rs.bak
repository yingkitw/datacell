...
impl CommandHandler for DefaultCommandHandler {
    fn handle(&self, command: Commands) -> Result<()> {
        match command {
            ...
            Commands::Write { output, csv, sheet } => self.handle_write(output, csv, sheet),
            Commands::Convert {
                input,
                output,
                sheet,
            ...
            Commands::Drop {
                ...
                output,
                columns,
            } => self.handle_drop(input, output, columns),
            Commands::Fillna {
                input,
                output,
                value,
            ...
            Commands::GeoDistance { from, to, unit } => self.handle_geo_distance(from, to, unit),
            Commands::Encrypt {
                input,
                output,
                key,
            ...
            Commands::Pipeline { config } => self.handle_pipeline(config),
            Commands::ApiServer { host, port, cors } => self.handle_api_server(host, port, cors),
            Commands::Plugin {
                function,
                input,
                output,
...
impl DefaultCommandHandler {
    ...
    fn handle_formula(
        ...
        println!(
            "Formula command: {} {} {} {} {:?}",
            input, output, formula, cell, sheet
        );
        Ok(())
    }
    ...
    fn handle_replace(
        ...
        println!(
            "Replace command: {} {} {} {} {:?}",
            input, output, find, replace, column
        );
        Ok(())
    }
    ...
    fn handle_astype(
        ...
        println!(
            "Astype command: {} {} {} {}",
            input, output, column, target_type
        );
        Ok(())
    }
    ...
    fn handle_pivot(
        ...
        println!(
            "Pivot command: {} {} {} {} {} {}",
            input, output, index, columns, values, agg
        );
        Ok(())
    }

    fn handle_parse_date(
        &self,
        input: String,
        output: String,
        column: String,
    ...
    fn handle_batch(
        ...
        println!(
            "Batch command: {} {} {} {}",
            inputs, output_dir, operation, args
        );
        Ok(())
    }
    ...
    fn handle_export_styled(
        ...
        let mut options = crate::excel::WriteOptions::default();
    ...
    fn handle_chart(
        ...
        println!(
            "Chart command: {} {} {} {:?} {:?} {:?}",
            input, output, chart_type, title, x_column, y_column
        );
        Ok(())
    }
    ...
    fn handle_validate(
        ...
        let data: Vec<Vec<String>> = data_str
            .lines()
            .filter(|l| !l.is_empty())
    ...
    fn handle_resample(
        ...
        println!(
            "Resampled {} data points to {} using {} aggregation",
            timeseries.len(),
            resampled.len(),
            aggregation
        );
    ...
    fn handle_text_analysis(
        ...
        match operation.as_str() {
            ...
            _ => {
                anyhow::bail!(
                    "Unknown operation: {}. Available: stats, sentiment, keywords, language",
                    operation
                );
            }
        }
    ...
    fn handle_detect_anomalies(
        ...
        let anomaly_method = match method.to_lowercase().as_str() {
            ...
            "percentile" => AnomalyMethod::Percentile {
                lower: 5.0,
                upper: 95.0,
            },
    ...
    fn handle_plugin(
        ...
        println!(
            "Plugin '{}' executed successfully. Output written to {}",
            function, output
        );
    ...
    fn handle_stream(&self, input: String, output: String, chunk_size: usize) -> Result<()> {
        ...
        while reader.has_more() {
            if let Some(chunk) = reader.read_chunk(chunk_size)? {
                ...
                chunks_processed += 1;
                println!(
                    "Processed chunk {} ({} rows)",
                    chunk.sequence, chunk.metadata.row_count
                );
            }
        }
        ...
        println!(
            "Streaming completed: {} chunks processed, {} total rows",
            chunks_processed,
            all_data.len()
        );
    ...
    fn find_column_index(&self, data: &[Vec<String>], column: &str) -> Result<usize> {
        ...
        if column.chars().all(|c| c.is_ascii_alphabetic()) && column.len() <= 3 {
            let idx = self.parse_column_ref(column)?;
            if idx < header.len() {
                return Ok(idx);
            }
        }
    ...
    fn parse_column_ref(&self, col: &str) -> Result<usize> {
    ...
    fn parse_resample_interval(
        &self,
        interval: &str,
    ) -> Result<crate::timeseries::ResampleInterval> {
...
#[cfg(test)]
mod tests {
    use super::DefaultCommandHandler;

    #[test]
    fn parses_excel_column_letters() {
        let handler = DefaultCommandHandler::new();
        assert_eq!(handler.parse_column_ref("A").unwrap(), 0);
        assert_eq!(handler.parse_column_ref("Z").unwrap(), 25);
        assert_eq!(handler.parse_column_ref("AA").unwrap(), 26);
        assert_eq!(handler.parse_column_ref("AB").unwrap(), 27);
        assert_eq!(handler.parse_column_ref("ba").unwrap(), 52);
    }
}
