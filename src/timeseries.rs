...
impl TimeSeriesProcessor {
    ...
    pub fn csv_to_timeseries(
        ...
        for (i, row) in data.iter().enumerate().skip(1) { // Skip header
            if let (Some(date_str), Some(value_str)) = (row.get(date_col), row.get(value_col)) {
                let timestamp = self.parse_date(date_str)?;
                let value = string::to_number(value_str)
                    .ok_or_else(|| anyhow::anyhow!("Invalid number at row {}: {}", i + 1, value_str))?;
    ...
    fn get_interval_key(&self, timestamp: NaiveDateTime, interval: &ResampleInterval) -> NaiveDateTime {
        match interval {
            ResampleInterval::Daily => {
                timestamp.date().and_hms_opt(0, 0, 0).unwrap()
            }
            ResampleInterval::Weekly => {
                let week_start = timestamp.date() - Duration::days(timestamp.weekday().num_days_from_sunday() as i64);
            ...
            ResampleInterval::Yearly => {
                NaiveDate::from_ymd_opt(timestamp.year(), 1, 1)
                    .unwrap()
                    .and_hms_opt(0, 0, 0)
                    .unwrap()
            }
            ResampleInterval::Hourly => {
                timestamp.date().and_hms_opt(timestamp.hour(), 0, 0).unwrap()
            }
            ResampleInterval::Minute => {
                timestamp.date().and_hms_opt(timestamp.hour(), timestamp.minute(), 0).unwrap()
            }
            ResampleInterval::Custom(duration) => {
                ...
                let rounded_duration = (duration_since_epoch.num_seconds() as i64 / duration.num_seconds()) * duration.num_seconds();
    ...
    pub fn rolling_mean(&self, data: &[TimeSeriesPoint], window: &RollingWindow) -> Result<Vec<TimeSeriesPoint>> {
        if data.len() < window.min_periods {
            return Ok(Vec::new());
        }
        
        let mut result = Vec::new();
        let window_size_secs = window.window_size.num_seconds() as usize;
    ...
    pub fn detect_trend(&self, data: &[TimeSeriesPoint]) -> TrendDirection {
        ...
        let xy_sum: f64 = data.iter().enumerate().map(|(i, p)| i as f64 * p.value).sum();
    ...
    fn detect_seasonality(&self, data: &[TimeSeriesPoint]) -> bool {
        ...
        for point in data {
            ...
            monthly_data.entry(month).or_insert_with(Vec::new).push(point.value);
        }
        ...
        let variance = monthly_means.iter().map(|m| (m - mean).powi(2)).sum::<f64>() / monthly_means.len() as f64;
    ...
    fn test_detect_trend() {
        ...
        let increasing_data: Vec<TimeSeriesPoint> = (0..10)
            .map(|i| TimeSeriesPoint {
                timestamp: NaiveDate::from_ymd_opt(2023, 1, 1).unwrap()
                    .and_hms_opt(0, 0, 0)
                    .unwrap()
                    + Duration::days(i),
                value: i as f64,
