...
impl TextAnalyzer {
    ...
    pub fn analyze_sentiment(&self, text: &str) -> SentimentResult {
        ...
        let (sentiment, confidence) = if positive_score > negative_score && positive_score > neutral_score {
    ...
    fn calculate_language_scores(&self, words: &[String]) -> Vec<(String, f64)> {
        ...
        let language_indicators = [
            ("english", vec!["the", "and", "is", "in", "to", "of", "a", "that", "it", "with"]),
            ("spanish", vec!["el", "la", "de", "que", "y", "en", "un", "es", "se", "no"]),
            ("french", vec!["le", "de", "et", "à", "un", "il", "être", "et", "en", "avoir"]),
            ("german", vec!["der", "die", "und", "in", "den", "von", "zu", "das", "mit", "sich"]),
        ];
    ...
    fn default_stop_words() -> HashSet<String> {
        vec![
            ...
            "they", "this", "to", "was", "will", "with", "the", "this", "is", "at", "which", "on",
        ]
        .into_iter()
        .map(|s| s.to_string())
        .collect()
    ...
    fn default_sentiment_words() -> SentimentWords {
        let positive = vec![
            ...
            "outstanding", "superb", "magnificent", "marvelous", "terrific", "splendid", "remarkable",
            "exceptional", "perfect", "love", "like", "enjoy", "happy", "pleased", "satisfied", "delighted",
            "thrilled", "excited", "joyful", "cheerful", "positive", "optimistic", "hopeful", "confident",
        ];
        
        let negative = vec![
            "bad", "terrible", "awful", "horrible", "dreadful", "appalling", "disgusting", "atrocious",
            "abysmal", "deplorable", "regrettable", "unfortunate", "disappointing", "sad", "angry", "frustrated",
            "annoyed", "upset", "worried", "concerned", "anxious", "nervous", "stressed", "unhappy", "miserable",
            "depressed", "negative", "pessimistic", "hopeless", "desperate", "failed", "failure", "wrong",
        ];
...
mod tests {
    ...
    fn test_sentiment_analysis() {
        ...
        assert!(positive_result.positive_score > positive_result.negative_score,
        ...
        assert!(negative_result.negative_score > negative_result.positive_score,
                "Negative text should have higher negative score. Positive: {}, Negative: {}",
                negative_result.positive_score, negative_result.negative_score);
        
        // If scores are clear, check sentiment classification
        if positive_result.positive_score > positive_result.neutral_score {
            assert!(matches!(positive_result.sentiment, Sentiment::Positive),
                    "Expected Positive sentiment, got {:?}", positive_result.sentiment);
        }
        
        if negative_result.negative_score > negative_result.neutral_score {
            assert!(matches!(negative_result.sentiment, Sentiment::Negative),
                    "Expected Negative sentiment, got {:?}", negative_result.sentiment);
        }
    }
