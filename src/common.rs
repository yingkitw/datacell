...
pub mod format {
    ...
    pub fn is_supported(format: &str) -> bool {
        matches!(format, "csv" | "excel" | "ods" | "parquet" | "avro" | "json")
    }
}
...
pub mod validation {
    ...
    pub fn validate_column_index(data: &[Vec<String>], col_idx: usize) -> Result<()> {
        ...
        if col_idx >= num_cols {
            return Err(anyhow!("Column index {} out of range (max: {})", col_idx, num_cols - 1));
        }
...
pub mod transform {
        ...
        F: FnMut(&str) -> String,
    {
        ...
        for row in data.iter_mut().skip(1) { // Skip header
