...
impl DataProfiler {
    ...
    fn calculate_text_stats(&self, data: &[String]) -> TextStats {
        ...
        let contains_special_chars = non_empty.iter().any(|v| {
            v.chars()
        ...
        for text in &non_empty {
            if text.chars().all(|c| c.is_uppercase() || !c.is_alphabetic()) {
                ...
                all_lowercase += 1;
            } else if text.chars().enumerate().all(|(i, c)| {
                (i == 0 && c.is_uppercase()) || (i > 0 && c.is_lowercase()) || !c.is_alphabetic()
            }) {
                title_case += 1;
            } else {
                mixed_case += 1;
...
mod tests {
    ...
    fn test_data_type_inference() {
        ...
        // Test numeric
        let numeric_data: Vec<String> = vec!["1", "2", "3", "4.5", "-1.2"]
            .into_iter()
            .map(|s| s.to_string())
            .collect();
        assert!(matches!(
        ...
        // Test integer
        let int_data: Vec<String> = vec!["1", "2", "3", "4", "-1"]
            .into_iter()
            .map(|s| s.to_string())
            .collect();
        assert!(matches!(
        ...
        // Test email
        let email_data: Vec<String> = vec!["test@example.com", "user@domain.org"]
            .into_iter()
            .map(|s| s.to_string())
            .collect();
        assert!(matches!(
        ...
        // Test boolean
        let bool_data: Vec<String> = vec!["true", "false", "yes", "no"]
            .into_iter()
            .map(|s| s.to_string())
            .collect();
        assert!(matches!(
